...
另外，合并到主线 AOSP 的内容和保存在国家安全局比特桶存储库中的内容有点不同。国家安全局在这些政策文件中有额外的控制，可以导致应用安装中止。可以说，谷歌和美国国家安全局在这个问题上“分叉”了。在`SELinuxMMAC.java`的 NSA 版本中，您可以指定匹配特定签名或包名的应用被允许拥有某些安卓级别的权限集。例如，您可以阻止安装所有请求`CAMERA`权限的应用，或者阻止使用特定密钥签名的应用。这也强调了在大型代码库中找到补丁并快速了解项目如何发展是多么重要，这通常看起来令人望而生畏。
