...
记得启用 minify。否则，资源萎缩将不起作用。这在我们使用外部库的地方非常有用，但是并不是所有的资源都被使用了。例如，如果我们已经将GooglePlay服务库添加到我们的项目中，但是我们没有使用谷歌+登录，或者谷歌 Cast 应用编程接口，那么 Gradle 将从结果文件中删除相关的未使用的资源。
