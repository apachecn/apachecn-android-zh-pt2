...
现在我们可以创建一个跨平台的应用，在 PC 上调试，部署到安卓设备上。但是，我们不能在GooglePlay上上传它，因为它还没有用发布密钥正确签名。
...
在我们可以安全地在GooglePlay上发布我们的`.apk`包之前，还有一步。安卓应用可以使用`mmap()`调用在`.apk`内访问未压缩的内容。然而，`mmap()`可能意味着对底层数据的一些对齐限制。我们需要在 4 字节的边界上对齐`.apk`内的所有未压缩数据。Android SDK 有`zipalign`工具可以做到，如下命令所示:
