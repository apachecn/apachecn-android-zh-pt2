...
在Corona使用物理引擎相当容易。您使用显示对象，并将它们设置为代码中的物理实体。图像、精灵和矢量形状可以变成物理对象。这对于可视化您希望您的对象在您创建的环境中的反应非常重要。你可以马上看到结果，而不是猜测它们在物理世界中的行为。
...
Corona模拟器中没有加速度计；必须创建设备构建才能看到效果。

### 物理学

该功能设置用于在屏幕Corona坐标和模拟物理坐标之间转换的内部每米像素比率。这应该在任何物理对象被实例化之前完成。
...
默认方向设置决定了设备的初始发射方向和Corona模拟器的初始方向。
...
Corona 可以针对 iOS 和 Android 上的多个设备制作的构件，这些构件以不同的分辨率显示各种艺术资产。Corona可以根据您的起始分辨率向上或向下缩放。它还可以在需要时替换更高分辨率的图像文件，确保您的应用在所有设备上看起来清晰锐利。
...
8.  加速度计事件只能在设备上测试，因此我们将通过调用`"simulator"`环境来为拨片上的触摸事件添加一个变量。这是为了让我们可以在Corona模拟器中测试桨的运动。如果你要在一个设备上测试应用，触摸的事件监听器和拨片上的加速度计不会冲突:
...
我们在这个游戏中添加的新的Corona应用编程接口是物理引擎。我们将向主要游戏对象(桨、球和砖块)添加物理参数，用于碰撞检测。拥有`setGravity(0,0)`将允许球在运动场周围自由弹跳。
