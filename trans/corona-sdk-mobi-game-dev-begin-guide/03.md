...
Corona 可以针对 iOS 和 Android 上的多个设备制作的构件，这些构件以不同的分辨率显示各种艺术资产。电晕可以根据您的起始分辨率向上或向下缩放。它还可以在需要时替换更高分辨率的图像文件，确保您的应用在所有设备上看起来清晰锐利。
...
要启动任何应用，我们需要创建一个`main.lua`文件。这已经在[第 2 章](02.html "Chapter 2. Lua Crash Course and the Corona Framework")、 *Lua 速成班和 Corona 框架*中讨论过了，当时我们使用了一些示例代码并用模拟器运行它。
...
8.  加速度计事件只能在设备上测试，因此我们将通过调用`"simulator"`环境来为拨片上的触摸事件添加一个变量。这是为了让我们可以在电晕模拟器中测试桨的运动。如果你要在一个设备上测试应用，触摸的事件监听器和拨片上的加速度计不会冲突:
