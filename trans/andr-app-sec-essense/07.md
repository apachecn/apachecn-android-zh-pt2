...
正如您将在前面的代码片段中注意到的那样，`getExternalFilesDir()`方法接受一个参数。此参数用于根据介质类型识别适当的存储目录。例如，存储图片使用`ENVIRONMENT.DIRECTORY_PICTURES`，存储音乐文件使用`ENVIRONMENT.DIRECTORY_MUSIC`。如果这样的目录不存在，它将被创建，然后文件将被存储在那里。值`null`是应用程序的根目录。
