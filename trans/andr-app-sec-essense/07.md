...
如果应用需要缓存数据，谨慎的做法是使用Android 栈提供的缓存存储机制。安卓将缓存文件与应用一起存储在文件系统中，以便它们与创建它的应用一起被沙箱保护。所有缓存文件都是在`/data/data/<application-path>/cache/`目录下创建的。如果系统内存不足，将首先删除这些缓存文件。定期清理这些文件是必要的，因为它们可能会变大并耗尽磁盘空间。
...
*   `ACTION_EXTERNAL_APPLICATIONS_UNAVAILABLE`:SD卡卸载时发出此意向。它包含禁用应用列表(使用`EXTRA_CHANGED_PACKAGE_LIST`属性)和不可用应用指南列表(使用`EXTRA_CHANGED_UID_LIST`属性)。
