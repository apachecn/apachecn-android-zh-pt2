...
![Passing data between Fragments](graphics/B05057_05_01.jpg)

这是当你点击一个国家名称时显示细节片段的屏幕:

![Passing data between Fragments](graphics/B05057_05_02.jpg)

当处于横向时，两个碎片会并排，如横向截图所示:

![Passing data between Fragments](graphics/B05057_05_03.jpg)
...
![Creating a shortcut on the Home screen](graphics/B05057_05_04.jpg)
...
应用程序小部件的基础知识已经涵盖，现在是时候开始编码了。我们的示例将涵盖基础知识，以便您可以根据需要扩展功能。这个食谱使用了一个带时钟的视图，当按下时，打开我们的活动。

此屏幕截图显示了添加到主屏幕时小部件列表中的小部件:

![Creating a Home screen widget](graphics/B05057_05_05.jpg)
...
![Creating a Home screen widget](graphics/B05057_05_06.jpg)
...
`AppWidgetProvider`类是我们处理由`updatePeriodMillis`投票触发的`onUpdate()`事件的地方。我们的示例不需要任何更新，因此我们将轮询设置为零。最初放置小部件时，仍会调用更新。`onUpdate()`是我们在按下时钟时设置打开应用程序的待定意图的地方。
...
![Adding Search to the Action Bar](graphics/B05057_05_07.jpg)

此屏幕截图显示了按下时搜索选项如何扩展:

![Adding Search to the Action Bar](graphics/B05057_05_08.jpg)
...
请注意，这是在应用程序元素中，而不是在两个`<activity>`元素中。
