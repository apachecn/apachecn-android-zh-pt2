...
材料设计指南规范的一部分讨论了应用中的文本选择。用户在你的应用中选择文本，你现在有了一个应用编程接口来合并一个**浮动工具栏**设计模式，类似于一个上下文操作栏。有关设计规格的更多信息，请前往[http://www . Google . com/design/spec/patterns/selection . html # selection-item-selection](http://www.google.com/design/spec/patterns/selection.html#selection-item-selection)。
