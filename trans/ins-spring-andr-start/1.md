...
欢迎来到安卓入门版的即时*春天。这本书是专门为您提供使用 Spring for Android 轻松开发 Android 所需的所有信息而编写的。您将了解 Spring 为 Android 提供的不同功能，并开始使用这些功能构建您的第一个应用。*

这本书包含以下几节:

*那么，安卓的春天是什么？*描述 Spring for Android 对 Spring Framework 进行了扩展，有助于简化原生 Android 应用的开发；它目前(版本 1.0.0.RELEASE)具有 REST 客户端(RestTemplate)和 OAuth 支持(Spring Social)。
