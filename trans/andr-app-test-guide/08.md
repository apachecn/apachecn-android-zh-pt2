...
如果我们的目标是将**持续集成**结合到我们的开发过程中，第一步将是手动构建安卓应用，因为我们可以将其与这种技术相结合来自动化该过程。
...
这里值得注意的是，整个安卓平台是由极其复杂的 makefiles 结构构建的，这种方法甚至用于构建平台包含的应用，如计算器、联系人、浏览器等。
...
| 调试 | 构建应用并用调试密钥对其进行签名。 |
| 释放；排放；发布 | 构建应用。生成的 `.apk`文件在发布前必须签名。 |
| 安装 | 将调试包安装/重新安装到正在运行的仿真器或设备上。如果应用是以前安装的，签名必须匹配。 |
| 从计算机上卸载 | 从正在运行的模拟器或设备卸载应用。 |
...
下一章讨论测试的另一个方面，集中在性能和概要分析上，这可能是在我们的应用按照我们的规范正确运行之后自然要遵循的步骤。