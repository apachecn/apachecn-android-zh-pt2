...
我们创建了一个非常简单的应用来演示本章中一些测试的使用。这个应用的源码可以从[http://www.packtpub.com/support](http://www.packtpub.com/support)下载。

下一个屏幕截图显示该应用正在运行:

![The demonstration application](img/3500_03_01%282%29.jpg)
...
![Static imports](img/3500_03_02.jpg)
...
在这种特殊情况下，我们在不区分大小写的匹配结果中查找单词“ERROR”(为此设置标志“I”)。也就是说，如果由于某种原因大写在我们的应用中不起作用，并且它包含一条错误消息，我们将通过断言检测到这种情况。
...
在某些情况下，我们所需要的只是能够提供一个到文件和数据库操作的替代路径。例如，如果我们在真实的设备上测试应用，也许我们不想在测试过程中影响现有的文件。
...
假设您的应用使用了一个 `ContentProvider`，可能来自多个 `Activity`。您可以使用 `ProviderTestCase2`为这个 `ContentProvider`创建单元测试，我们将很快看到这一点，并且在某些情况下实现一个如前所述的 `RenamingDelegatingContext`。
...
![The TestCase base class](img/3500_03_03.jpg)
...
![The AndroidTestCase base class](img/3500_03_04.jpg)
...
在运行任何应用代码之前，系统会实例化工具，从而允许它监控系统和应用之间的所有交互。

与许多其他安卓应用组件一样，工具实现在标签`<instrumentation>`下的 `AndroidManifest.xml`中描述。例如，如果你打开我们的测试‘ `AndroidManifest.xml`,看看里面你会发现:
...
如前所述，Instrumentation 类用于监控系统和被测应用或活动之间的交互。内部类 `Instrumentation.ActivityMonitor`允许监控应用中的单个活动。
...
![The InstrumentationTestCase class](img/3500_03_05.jpg)
...
在等待应用空闲之前，我们使用 `Instrumentation.waitForIdleSync()`请求 `mMessage EditText`的焦点，然后向其发送密钥序列。之后，使用 `TouchUtils.clickView()`，点击 `Button`，最终查看转换后的字段内容。
...
![The ProviderTestCase2<T> class](img/3500_03_08.jpg)
...
![Using external libraries in test projects](img/3500_03_10.jpg)
...
![Using external libraries in test projects](img/3500_03_11.jpg)
...
*   使用了几种类型的断言，从通常在 JUnit 测试中发现的最常见的断言，到在 Android SDK 中发现的最专业的断言，来运行应用 ui
*   解释了模拟对象及其在安卓测试中的使用
